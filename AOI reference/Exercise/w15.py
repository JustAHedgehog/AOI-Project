...
def camCaptureChessBoard():
    ...
    while a:
        ...
        if cv2.waitKey(1) & 0xff==ord("o"):
            ret, frame=cap.read()
            if ret==True:
                cv2.imwrite("D:/w15/testBoard%d.jpg"%i,frame)
                print (i)
                i=i+1
...
def CBxyzNuv():
    objp = np.zeros((6*7,3), np.float32)# prepare object points, like (0,0,0), (1,0,0), (2,0,0) ....,(6,5,0)
    objp[:,:2] = np.mgrid[0:7,0:6].T.reshape(-1,2) #世界座標
    
    # Arrays to store object points and image points from all the images.
    objpoints = [] # 3d point in real world space
    imgpoints = [] # 2d points in image plane.
    images = glob.glob('D:/w15/testBoard*.jpg')
    
    for fname in images:
        img = cv2.imread(fname)
        gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        ret, corners = cv2.findChessboardCorners(gray, (7,6))    # Find the chess board corners
        # 輸入輸入棋盤格灰階圖片，和棋盤內角點數
...
def camClibration (objpoints, imgpoints, imgshape):
    ...
    np.save("D:/w15/M.npy",mtx)
    np.save("D:/w15/D.npy",dist)
...
if __name__=="__main__":
    ...
    mtx=np.load("D:/w15/M.npy")
    dist=np.load("D:/w15/D.npy")
